...
def visualize(
    ...
    img_composite = Image.composite(
        to_pil_image(imgs_grid),
        to_pil_image(unmpatched_grid),
        to_pil_image(mask_grid).convert(mode="L"),
    )
...
def save_visualisations(
    vis_tuple: Tuple[Image.Image], prefix: str = None, folder: str = None
) -> None:
...
def train_mae(data_dir: str):
    ...
    optimizer = optim.AdamW(
        mae.parameters(), lr=lr, betas=(0.9, 0.95), weight_decay=0.05
    )
